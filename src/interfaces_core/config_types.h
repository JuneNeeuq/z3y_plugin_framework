...
enum class WidgetType {
  ...
  kCustom         /**< 自定义UI逃生舱：交由前台注册的闭包工厂进行专属渲染 */
};
...
struct SchemaMetadata {
  std::string name_key; /**< @brief 配置项名称 (支持多语言 Key，例如
                           "Camera.Exposure.Name") */
  std::string group_key; /**< @brief 一级分组名 (通常映射为 UI 的 Tab 页签) */
  std::string
      subgroup_key; /**< @brief 二级分组名 (通常映射为 UI 的 GroupBox 框) */
  std::string
      tooltip_key; /**< @brief 鼠标悬浮提示 (Tooltip，解释该参数的具体作用) */
  std::string enable_condition; /**< @brief UI 联动表达式，例如 "ParamA ==
                                   1"，满足时此控件才启用 */

  ConfigValue
      min_val; /**< @brief 允许的最小值，如果类型为 monostate 表示无限制 */
  ConfigValue max_val; /**< @brief 允许的最大值，超出将遭到后端拦截并报错 */
  ConfigValue
      step_val; /**< @brief 步进值，用于 UI 控件 (如 SpinBox) 的单次点击增量 */

  std::vector<std::string> enum_values; /**< @brief 下拉框后台真实值列表 */
  std::vector<std::string> enum_display_keys; /**< @brief 下拉框前端显示文本列表
                                                 (与 values 必须一一对应) */

  std::string file_filter; /**< @brief 文件选择器过滤器，例如 "*.json *.txt" */
  std::string permission_token; /**< @brief 权限令牌，例如
                                   "admin_only"，校验失败拒绝写入 */

  WidgetType widget_type =
      WidgetType::kAuto; /**< @brief 强制指定的 UI 控件呈现形式 */

  bool is_advanced =
      false; /**< @brief 是否为高级参数 (UI 默认隐藏，需勾选“高级模式”才显示) */
  bool is_hidden =
      false; /**< @brief 是否为隐藏参数 (纯后端使用，UI 完全不渲染) */
  bool read_only =
      false; /**< @brief 是否只读 (UI 显示为禁用状态，通常用于监控状态) */
  bool requires_restart =
      false; /**< @brief 修改后是否需要重启软件才能生效 (UI 负责弹窗提示) */
